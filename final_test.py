...
def test_current_backend():
    ...
    try:
        ...
        print(f"SUCCESS Basic API: {response.json()['message']}")
    ...
    try:
        ...
        if result['success']:
            print(f"SUCCESS Image generation: {result['message']}")
            print(f"  - Image type: {result['image_data'][:50]}...")
        else:
            print(f"ERROR Image generation failed: {result['message']}")
    except Exception as e:
    ...
    try:
        ...
        result = response.json()
        if result['success']:
            print(f"SUCCESS Description generation: {result['message']}")
            print(f"  - Description: {result['description'][:100]}...")
            print(f"  - Aspects: {list(result.get('modular_description', {}).keys())}")
        else:
            print(f"ERROR Description generation failed: {result['message']}")
    except Exception as e:
...
def test_ai_services():
    ...
    try:
        ...
        print(f"SUCCESS Ollama running: {len(models.get('models', []))} models available")
    ...
    try:
        ...
        print(f"SUCCESS ComfyUI running: {response.json()}")
    except Exception as e:
        print(f"ERROR ComfyUI not accessible: {e}")
...
def test_frontend():
    ...
    try:
        ...
        if response.status_code == 200:
            print("SUCCESS Frontend accessible (200 OK)")
        elif response.status_code == 404:
            print("SUCCESS Frontend running but no specific route (404 normal for Next.js)")
        else:
            print(f"INFO Frontend responded with: {response.status_code}")
    except Exception as e:
...
def test_full_integration():
    ...
    try:
        ...
        if result['success']:
            print(f"SUCCESS Full integration successful: {result['message']}")
            if 'image_data' in result:
                print(f"  - Generated image: {result['image_data'][:50]}...")
            if 'description' in result:
                print(f"  - Generated description: {result['description'][:100]}...")
        else:
            print(f"ERROR Full integration failed: {result.get('message', 'Unknown error')}")
            
    except Exception as e:
        print(f"ERROR Full integration error: {e}")
...
def check_dependencies():
    ...
    dependencies = {
        ...
        "Ollama": "Check if accessible on port 11434",
    ...
    for name, command in dependencies.items():
        if name == "Python":
            ...
            try:
                ...
                print(f"SUCCESS {name}: Running with models")
            except:
            ...
            try:
                ...
                print(f"SUCCESS {name}: Running on port 8000")
            except:
                print(f"ERROR {name}: Not accessible")
        elif name == "Frontend":
            try:
                response = requests.get("http://localhost:3000", timeout=5)
                print(f"SUCCESS {name}: Running on port 3000")
            except:
                print(f"ERROR {name}: Not accessible")
